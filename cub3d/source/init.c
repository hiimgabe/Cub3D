...
/*   Updated: 2024/10/21 19:40:58 by gabe             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	if (!game()->map)
		error_exit("struct map error", NULL);
	(game())->texture_info = ft_calloc(1, sizeof(t_texture_info));
	if (!game()->texture_info)
		error_exit("struct textures info error", NULL);
...
void	init_mlx()
{
	...
	game()->mlx = mlx_init();
	if (!game()->mlx)
		return (error_exit("Failed to init mlx.", NULL));
	game()->mlx_win = mlx_new_window(game()->mlx, SCREEN_W, SCREEN_H, "CUB3D");
	if (!game()->mlx_win)
		return (error_exit("Failed to init mlx window.", NULL));
