...
/*   Updated: 2024/10/24 17:08:24 by gamoreir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cell.x < 0 || cell.y < 0 || cell.x > get_map_width(cell.y)
		|| cell.y > get_map_height())
		return (false);
	return (true);
}
...
int	check_move(t_pos move)
{
	...
	moved = 0;
	if (!is_map_cell(next)
		|| is_wall(game()->map->layout[(int)next.y][(int)next.x]))
		return (moved);
